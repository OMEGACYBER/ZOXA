...
function testEmotionalStateExposure() {
  ...
  apiFiles.forEach(file => {
    if (fs.existsSync(file)) {
      ...
      // Check for debug objects in response (only active console.log, not commented)
      const lines = content.split('\n');
      let hasActiveConsoleLog = false;
      let hasResJson = false;
      
      lines.forEach(line => {
        const trimmedLine = line.trim();
        if (trimmedLine.startsWith('console.log') && !trimmedLine.startsWith('//')) {
          hasActiveConsoleLog = true;
        }
        if (trimmedLine.includes('res.json')) {
          hasResJson = true;
        }
      });
      
      if (hasActiveConsoleLog && hasResJson) {
        violations.push({
          file,
